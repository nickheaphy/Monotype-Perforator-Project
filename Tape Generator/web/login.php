...
// Check if the user is already logged in, if yes then logout
if (isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $stm = $db->prepare("SELECT id, email, pwd FROM users WHERE email = ?");
        $stm->bindValue(1, $inputEmail);
        $res = $stm->execute();
    ...
    <main>
        ...
        <div class="container">
            <form class="form-signin" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                ...
                <input name="inputEmail" type="email" id="inputEmail" class="mb-3 form-control" placeholder="Email address" required autofocus>
                <label for="inputPassword" class="sr-only">Password</label>
                <input name="inputPassword" type="password" id="inputPassword" class="mb-3 form-control" placeholder="Password" required>
